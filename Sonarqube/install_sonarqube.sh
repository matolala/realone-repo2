...
# Set ulimits in profile (since ulimit can't be set in sysctl.conf)
sudo sh -c 'echo "ulimit -n 65536" >> /etc/profile'
sudo sh -c 'echo "ulimit -u 4096" >> /etc/profile'

sudo apt update -y
sudo apt install openjdk-17-jre-headless -y

# Postgres Database installation and setup
wget -q https://www.postgresql.org/media/keys/ACCC4CF8.asc -O - | gpg --dearmor | sudo tee /usr/share/keyrings/postgres.gpg > /dev/null
